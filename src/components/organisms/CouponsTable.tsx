...
const CouponsTableInner = ({
    ...
    const fmt = useCallback((d?: Date | null) => (d ? dateFmt.format(d) : "—"), [dateFmt]);

    const editFactory = useCallback((id: number) => () => onEdit(id), [onEdit]);
    const deleteFactory = useCallback((id: number) => () => onDelete(id), [onDelete]);
            ...
            <div className="hidden md:block overflow-x-auto">
                <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {coupons.map((c) => {
                            const discountPct = Math.round(c.discount * 100);
                            const activeBadge =
                                c.active
                            ...
                            return (
                                <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">{discountPct}%</td>
                                    <td className="px-6 py-4 text-sm">
                                        <span className={`px-2 py-1 rounded text-xs font-medium ${activeBadge}`}>
                                    ...
                                    <td className="px-6 py-4 text-sm text-right space-x-2">
                                        <button
                                            onClick={editFactory(c.id)}
                                            className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium transition-colors"
                                        >
                                            Edit
                                        </button>
                                        <button
                                            onClick={deleteFactory(c.id)}
...
export const CouponsTable = memo(CouponsTableInner);