                        ...
                        {orders.map((order) => (
                            <Fragment key={order.id}>
                                <tr className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{formatDate(order.placedAtUtc)}</td>
                                    <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">{order.items.length} item{order.items.length !== 1 ? "s" : ""}</td>
                                    <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{order.paymentMethod?.replace("_", " ") ?? "—"}</td>
                                    <td className="px-6 py-4 text-sm">
                                        <span className={getStatusBadge(order.paymentStatus)}>{order.paymentStatus ?? "Unknown"}</span>
                                    </td>
                                    <td className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-gray-100">{formatPrice(order.totalAmount)}</td>
                                    <td className="px-6 py-4 text-sm">
                                        <button
                ...
                {orders.map((order) => (
                    <div key={order.id} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                        ...
                        <div className="flex justify-between items-center mb-3">
                            ...
                            <span className="text-lg font-semibold text-gray-900 dark:text-gray-100">{formatPrice(order.totalAmount)}</span>
                        </div>
