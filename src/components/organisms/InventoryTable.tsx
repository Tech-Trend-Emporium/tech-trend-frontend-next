        ...
        <>
                ...
                <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {inventories.map((inv) => {
                            ...
                            const availabilityPct = pct(inv.available, inv.total);
                            return (
                                <tr key={inv.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">{inv.id}</td>
                                    <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">{inv.productName}</td>
                                    <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">{inv.total}</td>
                                    <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">{inv.available}</td>
                                    <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{reserved}</td>
                                    <td className="px-6 py-4 text-sm">
                                        <span className={`px-2 py-1 rounded text-xs font-medium ${availabilityPct > 50
            ...
            <div className="md:hidden divide-y divide-gray-200 dark:divide-gray-700">
                {inventories.map((inv) => {
                    ...
                    const availabilityPct = pct(inv.available, inv.total);
                    return (
                        <div key={inv.id} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                            <div className="flex justify-between items-start mb-3">
                                <div>
                                    <h3 className="font-semibold text-gray-900 dark:text-gray-100">
                                        {inv.productName}
                                    </h3>
                                    <p className="text-sm text-gray-600 dark:text-gray-400">ID: {inv.id}</p>
                                </div>
                            ...
                            <div className="flex justify-between items-center text-sm">
                                <div className="space-y-1 text-gray-600 dark:text-gray-400">
                                    ...
                                    <p>Availability: {availabilityPct}%</p>
                                </div>
                            </div>
                        </div>
                    );
